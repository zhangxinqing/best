<template>
	<view>
		...
		<movable-area class="filterimgbox" style="width:100%">
			<movable-view :style="[{width:windowWidth,height:windowWidth}]">
				<view :style="[{width:windowWidth,height:windowWidth}]">
				<img :src="orimg" :class="filteritem[filterindex].cls" :style="[{width:windowWidth,height:windowWidth}]" :mode="imgmode[imgmode_index]" />
				</view>
				</movable-view>
			...
			<movable-view class="moveitem mtag2" :x="good.x" :y="good.y" direction="all" @change="goodchange">(x:{{ good.x }},y:{{ good.y }}){{ good.title }}</movable-view>
		</movable-area>
		...
		<view class="cu-form-group">
			...
			<button class="cu-btn round sm" @tap="ChooseImage">图片</button>
			<button class="cu-btn round sm" @tap="changeMode">比例</button>
		</view>
...
export default {
	data() {
		return {
			imgmode_index:0,
			imgmode:['aspectFit','scaleToFill','aspectFill','top','bottom','center','left','right','top left','top right','bottom left','bottom right'],
			windowWidth:this.windowWidth+"px",
			uploadicon: '../../../static/upload.png',
			filterindex: 0,
	...
	methods: {
		...
		ChooseImage() {
			uni.chooseImage({
				...
				success: res => {
					this.orimg = res.tempFilePaths;
				}
			});
		},
		changeMode(){
			this.imgmode_index=this.imgmode_index+1;
			if(this.imgmode_index==4){
				this.imgmode_index=0;
			}
		}
